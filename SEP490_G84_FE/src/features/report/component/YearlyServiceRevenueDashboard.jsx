...
const YearlyServiceRevenueDashboard = ({ yearlyData, selectedYear, onMonthClick }) => {
    ...
    const best = data.reduce((a, b) => (a.revenue || 0) > (b.revenue || 0) ? a : b);
    const worst = data.reduce((a, b) => (a.revenue || 0) < (b.revenue || 0) ? a : b);

    const pieData = data
        .map(d => ({ name: d.monthLabel, value: d.revenue || 0 }))
        .filter(d => d.value > 0);
            ...
            <div className="d-flex justify-content-between align-items-end mb-3 border-bottom pb-2">
                <div>
                    <span className="badge rounded-pill px-3 py-2 mb-2 fw-bold"
                        style={{ backgroundColor: '#f0fdfa', color: ACCENT, border: `1px solid ${ACCENT}` }}>
                ...
                <div className="text-end">
                    <p className="text-secondary mb-1" style={{ fontSize: '0.8rem' }}>Total Yearly Service Revenue</p>
                    <h4 className="fw-bold m-0" style={{ color: ACCENT }}>{formatCurrency(totalRevenue)} VND</h4>
                </div>
            </div>
            ...
            <div className="row g-3 mb-4">
                <div className="col-md-3">
                    <div className="card shadow-sm border-0 rounded-4 p-3 h-100" style={{ borderLeft: `4px solid ${ACCENT}` }}>
                        ...
                        <h5 className="fw-bold text-dark mb-1">{formatCurrency(totalRevenue)}</h5>
                        <small className="text-muted">VND · 12-month cumulative</small>
                    </div>
                </div>
                <div className="col-md-3">
                    <div className="card shadow-sm border-0 rounded-4 p-3 h-100" style={{ borderLeft: '4px solid #5396ff' }}>
                ...
                <div className="col-md-3">
                    <div className="card shadow-sm border-0 rounded-4 p-3 h-100" style={{ borderLeft: '4px solid #f39c12' }}>
                        <p className="text-secondary text-uppercase fw-bold mb-1" style={{ fontSize: '0.72rem', letterSpacing: '1px' }}>BEST MONTH</p>
            ...
            <div className="row g-4 mb-4">
                ...
                <div className="col-lg-7">
                    <div className="card shadow-sm border-0 rounded-4 p-3 h-100">
                        <p className="text-secondary text-uppercase fw-bold mb-3" style={{ fontSize: '0.8rem', letterSpacing: '1px' }}>MONTHLY SERVICE REVENUE (VND)</p>
                        <div style={{ width: '100%', height: '280px' }}>
                            <ResponsiveContainer>
                                <ComposedChart data={data} margin={{ top: 10, right: 10, bottom: 5, left: 0 }}>
                                    ...
                                    <Bar yAxisId="left" name="Revenue" dataKey="revenue" radius={[4, 4, 0, 0]} barSize={22}
                                        onClick={(d) => onMonthClick(d.monthValue)} style={{ cursor: 'pointer' }}>
                                        {data.map((entry, index) => (
                                            <Cell key={`cell-${index}`}
                                                fill={entry.revenue === best.revenue ? ACCENT : '#a5d8d4'} />
                                        ))}
                                    </Bar>
                    ...
                    <div className="table-responsive">
                        <table className="table table-hover mb-0 text-center align-middle" style={{ borderColor: '#f0f0f0' }}>
                            ...
                            <tbody>
                                {data.map((m, i) => (
                                    <tr key={i}
                                        style={{ cursor: 'pointer', backgroundColor: m.revenue === best.revenue ? '#f0fdfa' : m.revenue === worst.revenue ? '#fafafa' : '#fff' }}
                                        onClick={() => onMonthClick(m.monthValue)}>
                                        <td className="text-start ps-4 fw-bold py-2 text-dark">
                                            {m.monthLabel}
                                            {m.revenue === best.revenue && <i className="bi bi-star-fill ms-1" style={{ fontSize: '0.75rem', color: ACCENT }}></i>}
                                        </td>
...
export default YearlyServiceRevenueDashboard;
