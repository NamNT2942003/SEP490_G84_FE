...
const Sidebar = () => {
    ...
    const menuItems = [
        ...
        {
            path: "/inventoryandfurnitureTest",
            label: "Item Inventory",
            icon: "bi-box",
            // Khai báo menu con ở đây (Đã sửa đường dẫn cho khớp AppRouter)
            subItems: [
                { path: "/inventoryandfurnitureTest/furniture", label: "Furniture Inventory" },
                { path: "/inventoryandfurnitureTest/history", label: "Import History" },
                { path: "/inventoryandfurnitureTest/report", label: "Inventory Report" }
            ]
        },
                                    ...
                                    <ul className="nav flex-column mt-1" style={{ paddingLeft: "2.5rem" }}>
                                        {item.subItems.map((sub, subIndex) => (
                                            <li className="nav-item mb-1" key={subIndex}>
                                                <NavLink
                                                    to={sub.path}
                                                    end={sub.path === "/inventoryandfurnitureTest"} // Đảm bảo trang gốc không bị highlight đè
                                                    className={({ isActive }) =>
